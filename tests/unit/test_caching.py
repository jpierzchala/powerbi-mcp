...
class TestPerModelCaching:
    ...
    def test_tables_cache_is_per_model_key(self):
        ...
        setattr(connector, "_get_model_last_update_marker", lambda: datetime(2024, 1, 1))

        # Model A
        connector.model_key = "endpoint|ModelA"
        setattr(
            connector.schema_service,
            "discover_tables",
            lambda: [{"name": "A_Table1", "description": "desc", "relationships": []}],
        )
        ...
        setattr(
            connector.schema_service,
            "discover_tables",
            lambda: [{"name": "B_Table1", "description": "desc", "relationships": []}],
        )
        ...
        setattr(
            connector.schema_service,
            "discover_tables",
            lambda: [{"name": "A_Table_SHOULD_NOT_BE_USED", "description": "desc", "relationships": []}],
        )
    ...
    def test_tables_cache_invalidates_on_marker_change(self):
        ...
        connector.model_key = "endpoint|ModelA"
        setattr(connector, "_get_model_last_update_marker", lambda: t1)
        setattr(
            connector.schema_service,
            "discover_tables",
            lambda: [{"name": "A_Table1", "description": "desc", "relationships": []}],
        ...
        setattr(
            connector.schema_service,
            "discover_tables",
            lambda: [{"name": "A_Table2", "description": "desc", "relationships": []}],
        )
